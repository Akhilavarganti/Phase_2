...
# Adafruit Blinka's top-level `board` module reads this file next to itself at
# runtime.  A hidden import includes board.py but does not include this data
# file, which otherwise causes the one-file executable to fail after unpacking.
BOARD_IMPORTS_JSON="$(python -c 'import board, pathlib; print(pathlib.Path(board.__file__).with_name("board_imports.json"))')"
if [[ ! -f "$BOARD_IMPORTS_JSON" ]]; then
    echo "Unable to locate board_imports.json: $BOARD_IMPORTS_JSON" >&2
    exit 1
fi
