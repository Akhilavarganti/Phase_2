...
class UDSApp:
    ...
    def git_setup(self):
        ...
        try:
            #branch = self.git.select_remote_branch()
            #logging.info(f"Repo Path:  {branch}")
    ...
    def initialize_dependencies(self):
        ...
        self.git = git_manager.GitManager(self)
        self.git_setup()
        # -------------------------------------------------
        # Create UDS object so UDP server can be checked
        # -------------------------------------------------
        self.uds = uds_client.UDSClient(self.config, config_path=self.selected_config, repo_path=self.git.repo_path)
        max_retries = 3
        udp_connected = False
        ...
        if can_config.get("bringup_on_startup", False):
            while time.time() - start_time < timeout_seconds:
                ...
                try:
                    ...
                    temp_uds = uds_client.UDSClient(self.config, repo_path=self.git.repo_path)
    ...
    def select_from_list(self, title, items, formatter=str, allow_cancel=False):
        ...
        while True:
            ...
            self.oled.display_centered_text(f"{title}\n\n{label}")
            action = self.poll_combined_action(options_count=min(len(items), 9), allow_cancel=allow_cancel, timeout=0.15)
            ...
            if action == "prev":
                ...
                return None
            elif isinstance(action, tuple) and action[0] == "index":
                return action[1]
