    ...
    def select_from_list(self, title, items, formatter=str, allow_cancel=False):
        ...
        while True:
            ...
            if index != last_displayed_index:
                ...
                if self.hardware_display is not None:
                    self.hardware_display.display_centered_text(f"{title}\n\n{label}")
        ...
        if self.has_terminal_input():
            self.print_terminal_instructions(
                [
                    ...
                    "Keypad: UP/DOWN select char, "
                    "OK adds char, BACK submits."
                ]
            )
        while True:
            current_char = chars[index]
            ...
            if display_key != last_display:
                if self.hardware_display is not None:
                    self.hardware_display.display_centered_text(
                        f"{title}\n"
                        f"{preview}\n\n"
                        f"[{current_char}]"
                    )
