    ...
    def run_testcase(self, oled):
        def wait_for_final_response(self, client, tc_id, step_desc):
            ...
            while response and response[0] == 0x7F and response[2] == 0x78:
                ...
                time.sleep(0.05)  
                # small delay (optional, reduced from 1s)
                # Check total elapsed time under P2*
                if time.time() - start_time > current_timeout:
                    logging.warning(f"{tc_id} {step_desc} -> Timed out waiting after 0x78 (P2*)")
                    break
            return response
                            ...
                            try:
                              ...
                              if subfunc != "":
                                ...
                                if subfunc_int == 0x01:
                                   ...
                                   client.conn.send(raw_request)
                                   response = wait_for_final_response(self, client, tc_id, step_desc)
