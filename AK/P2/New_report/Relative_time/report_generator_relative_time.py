...
import logging
from drivers.Parse_handler import HEADER_ALIASES, find_column, load_testcase_sequence
...
def generate_html_report(messages_by_tc, output_path, asc_file_path, start_ts, end_ts, ecu_info_data=None, target_ecu=None, expected_ecu_fields=None, author_name="N/A", input_file_metadata=None, testcase_order=None, ecu_info_field_map=None, asc_did_responses=None, tester_name="N/A", input_json_file=None, placeholder_messages=None, test_start_time=None, test_end_time=None, first_request_ts=None):
    ...
    # Relative timestamps from ASC log
    """if isinstance(start_ts, (int, float)) and isinstance(end_ts, (int, float)):
        relative_start_ts = start_ts
        relative_end_ts = end_ts
    else:
        relative_start_ts = 0.0
        relative_end_ts = 0.0"""
    
    if test_start_time:
        test_start_display = test_start_time.strftime("%H:%M:%S.%f")[:-3]
    else:
        test_start_display = "N/A"
        
    if test_end_time:
        test_end_display = test_end_time.strftime("%H:%M:%S.%f")[:-3]
    else:
        test_end_display = "N/A"
    ...
    app_version = "DTP-CAN Diagnostic Application V6.0.0"
...
<div class="overview-grid">
    <div>
        <table class="info-table">
            <tr><td>Test start time</td><td>{test_start_display} ({first_request_ts:.4f})</td></tr>
            <tr><td>Test end time</td><td>{test_end_display} ({end_ts:.6f})</td></tr>
...
def parse_asc_file_v2(asc_file_path, allowed_tx_ids, allowed_rx_ids, txt_file_path):
    ...
    for line in lines:
        ...
        if direction == "Tx" and msg["can_id"] in allowed_tx_ids:
                ...
                if len(request_builder["payload"]) >= request_builder["total_len"]:
                    ...
                    if definition:
                        ...
                        if first_request_ts is None:
                            first_request_ts = request_builder["timestamp"]
            ...
            if definition:
                ...
                if first_request_ts is None:
                    first_request_ts = msg["timestamp"]
            elif matcher.has_matched_payload(request_payload):
    ...
    return messages_by_tc, start_ts or 0, end_ts or 0, first_request_ts or 0

def generate_report(asc_file_path, txt_file_path, output_html_file, allowed_tx_ids, allowed_rx_ids, ecu_info_data=None, target_ecu=None, ecu_info_field_map=None, tester_name="N/A", input_json_file=None, test_start_time=None, test_end_time=None):
    ...
    messages_by_tc, start_ts, end_ts, first_request_ts = parse_asc_file_v2(
        asc_file_path,
        allowed_tx_ids,
        allowed_rx_ids,
        txt_file_path,
