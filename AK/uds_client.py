    ...
    def run_testcase(
        ...
        for tc_id, steps in grouped_cases.items():
            ...
            for step in steps:
                ...
                if delay:
                    try:
                        delay_str = str(delay).strip().lower()
                        if delay_str.endswith("ms"):
                            wait_time = float(delay_str[:-2]) / 1000
                        elif delay_str.endswith("s"):
                            wait_time = float(delay_str[:-1])
                        else:
                            wait_time = float(delay_str)
                        logging.info(
                                f"{tc_id} {step_desc} -> Waiting {wait_time} seconds"
                        )
                        time.sleep(wait_time)
                    except Exception as e:
                        logging.warning(
                            f"{tc_id} {step_desc} -> Invalid delay '{delay}': {e}"
                        )
                try:
