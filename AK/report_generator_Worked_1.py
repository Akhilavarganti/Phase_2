...
def parse_asc_file(asc_file_path, allowed_tx_ids, allowed_rx_ids):
    ...
    for line in lines:
        ...
        # ◀️ Rx: Handle Response
            ...
            messages_by_tc.append(current_request)
...
def generate_html_report(messages_by_tc, output_path, asc_filename, start_ts, end_ts, ecu_info_data=None, target_ecu=None, base_datetime=None):
    ...
    passed = sum(1 for msg in messages_by_tc if msg["status"] == "Pass")
    #failed = total - passed
    failed = total - passed
...
<div style="display: flex; justify-content: flex-start; align-items: flex-start; gap: 40px; margin-top: 20px; padding-left: 10px;">
    ...
    <hr><br>
    """
    for idx, msg in enumerate(messages_by_tc):
        tc_id = msg["tc_id"]
        # 👇 Detect if it's PreCondition (PC_ID)
        is_pc = tc_id.startswith("PC_ID")
        status = msg['status']
        status_class = 'pass' if status == 'Pass' else 'fail' if status == 'Fail' else 'pending'
        html += f"<div class='case-block {status_class}-case'>\n"
        if is_pc:
            html += f"<details open><summary><b>{tc_id}</b></summary>\n"\
        else:
        ...
        html += f"<tr><td>{step_count}</td><td>{combined_desc}</td><td>{msg['timestamp']:.6f}</td><td>Request Sent</td><td>{req_data_str}</td><td></td><td>-</td></tr>\n"
        step_count += 1
