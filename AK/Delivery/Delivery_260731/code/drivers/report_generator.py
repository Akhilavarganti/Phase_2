...
def parse_asc_trigger_datetime(date_str):
    for fmt in ("%a %b %d %H:%M:%S.%f %Y", "%a %b %d %I:%M:%S.%f %p %Y"):
        try:
...
def response_compare_variants(data_bytes):
    ...
    if len(payload_trimmed) >= 3 and payload_trimmed[0] == "62":
        did_and_data = payload_trimmed[1:]
        data_only = payload_trimmed[3:]
        if did_and_data and did_and_data not in variants:
            variants.append(did_and_data)
        if data_only and data_only not in variants:
            variants.append(data_only)

    if len(payload_trimmed) >= 2 and payload_trimmed[0] != "62":
        # Some ECUs respond as DID + data without the positive response SID.
        data_only = payload_trimmed[2:]
        if data_only and data_only not in variants:
            variants.append(data_only)

    if len(payload_trimmed) >= 2 and payload_trimmed[0] == "67":
        security_access_prefix = payload_trimmed[:2]
        if security_access_prefix not in variants:
            variants.append(security_access_prefix)
...
def get_status(actual_data, expected_response_data):
    ...
    expected_payload = expected_declared_payload(expected_response_data)
    actual_payload = trim_compare_padding(normalize_response_payload(actual_data))
    if (
        expected_payload
        and expected_payload[0] != "67"
        and len(actual_payload) < len(expected_payload)
        and expected_payload[: len(actual_payload)] == actual_payload
    ):
        return (
            "Fail",
            f"Response shorter than expected ({len(actual_payload)}/{len(expected_payload)} bytes)",
        )
...
def parse_asc_file(asc_file_path, allowed_tx_ids, allowed_rx_ids):
    ...
    for i, line in enumerate(lines):
        if line.startswith("Begin Triggerblock"):
            ...
            base_datetime = parse_asc_trigger_datetime(date_str)
            break
...
def generate_html_report(messages_by_tc, output_path, asc_file_path, start_ts, end_ts, ecu_info_data=None, target_ecu=None, base_datetime=None, expected_ecu_fields=None, author_name="N/A", input_file_metadata=None, testcase_order=None, ecu_info_field_map=None, asc_did_responses=None, tester_name="N/A", input_json_file=None, placeholder_messages=None):
    ...
    def testcase_status(steps):
        latest_by_request = OrderedDict()
        for msg in steps:
            request_key = tuple(
                msg.get("request_payload")
                or get_request_match_bytes(msg.get("data_bytes", []))
                or [msg.get("desc", "")]
            )
            latest_by_request[request_key] = msg

        latest_steps = list(latest_by_request.values()) or list(steps)
        if latest_steps and all(msg.get("status") == "Pass" for msg in latest_steps):
            return "Pass"
        return "Fail"
    ...
    def format_report_time(relative_ts):
        if base_datetime and isinstance(relative_ts, (int, float)):
            return (base_datetime + datetime.timedelta(seconds=relative_ts)).strftime(
                "%Y-%m-%d %H:%M:%S"
            )
        if isinstance(relative_ts, (int, float)):
            return f"{relative_ts:.6f}s"
        return "N/A"

    verdict_text = "Test passed" if failed == 0 else "Test failed"
    verdict_class = "overall-pass" if failed == 0 else "overall-fail"
    begin_time = format_report_time(start_ts)
    end_time = format_report_time(end_ts)
...
class TestcaseMatcher:
    def __init__(self, definitions):
        ...
        self.last_used_by_payload = {}

    def _mark_used(self, definition):
        self.used_indices.add(definition.index)
        self.cursor = max(self.cursor, definition.index + 1)
        if definition.request_payload:
            self.last_used_by_payload[tuple(definition.request_payload)] = definition
        return definition
    ...
    def match(self, request_payload):
        ...
        payload_key = tuple(request_payload)
        ...
        if payload_key in self.last_used_by_payload:
            return self.last_used_by_payload[payload_key]
...
def generate_report(asc_file_path, txt_file_path, output_html_file, allowed_tx_ids, allowed_rx_ids, ecu_info_data=None, target_ecu=None, ecu_info_field_map=None, tester_name="N/A", input_json_file=None):
    ...
    messages_by_tc, start_ts, end_ts, base_datetime = parse_asc_file_v2(
        asc_file_path,
        allowed_tx_ids,
        allowed_rx_ids,
        txt_file_path,
