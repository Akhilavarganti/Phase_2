        ...
        try:
            ...
            def compare_variants(payload):
                ...
                for candidate in (raw, normalized):
                    ...
                    if len(trimmed) >= 2 and trimmed[0] == 0x67:
                        security_access_prefix = trimmed[:2]
            ...
            if (
                expected_payload
                and expected_payload[0] != 0x67
                and len(actual_payload) < len(expected_payload)
                and expected_payload[: len(actual_payload)] == actual_payload
            ):
                failure_reason = (
                    f"Response shorter than expected "
                    f"({len(actual_payload)}/{len(expected_payload)} bytes)"
                )
                logging.warning(
                    f"-----------------------------------------------{tc_id} {step_desc} -> FAIL - {failure_reason}-----------------------------------------------"
                )
